...
import { verifyOtp, forgotPassword } from "@/api/account.api";
...
export default function OtpVerification() {
  ...
  const onSubmit = async (data: OtpFormData) => {
    ...
    if (!email) {
      setErrorMessage("Missing email. Please restart the password reset process.");
      setIsLoading(false);
      return;
    }
    
    if (!token) {
      // Token validation - proceed anyway if missing
    }
  ...
  return (
    <div className="login-container min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 relative">
      {/* Guest Navbar */}
      <GenericNavbar 
        config={createGuestNavbarConfig()}
        fullWidth={true}
        collapsed={false}
      />
