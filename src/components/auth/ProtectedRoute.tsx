 ...
 * @param allowedRoles - Array of role names that can access this route (e.g., ['Admin', 'AcademicStaff', 'AccountantStaff'])
...
export default function ProtectedRoute({ 
  ...
  if (requireAuth && allowedRoles.length > 0) {
    ...
    if (!hasAllowedRole) {
      // Redirect based on user's role or to login if no role
      if (userRole?.toLowerCase() === 'admin') {
        return <Navigate to="/admin/analytics" replace />;
      } else if (userRole === 'AcademicStaff' || userRole === 'AccountantStaff') {
        return <Navigate to="/staff/classes" replace />;
      } else {
